...
class GOSTSelfTxSignerTest {
    ...
    private lateinit var selfTxSigner: SelfTxSigner
    private lateinit var cryptoProSigner: CryptoProSigner
    private lateinit var publicKey: PublicKey

    @BeforeEach
    fun init() {
        keyStore = KeyStore.getInstance(JCSP.HD_STORE_NAME, JCSP())
    ...
    fun `should sign IssueTx`() {
        ...
        val signedTx = selfTxSigner.sign(signRequest)

        assertNotNull(signedTx.proofs)
        assertTrue(signedTx.proofs!!.isNotEmpty())
        assertFalse(signedTx.senderAddress == Address.EMPTY)
        assertFalse(signedTx.id == TxId.EMPTY)
    }
    ...
    fun `should sign ReissueTx`() {
        ...
        assertNotNull(signedTx.proofs)
        assertFalse(signedTx.proofs!!.isEmpty())
        assertFalse(signedTx.senderAddress == Address.EMPTY)
        assertFalse(signedTx.id == TxId.EMPTY)
    }
