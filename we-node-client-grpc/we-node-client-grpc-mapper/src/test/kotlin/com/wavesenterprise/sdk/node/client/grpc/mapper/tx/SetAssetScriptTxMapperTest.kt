...
class SetAssetScriptTxMapperTest {

    @Test
    fun `should map to SetAssetScriptTx (all fields)`() {
        val txVersion = TxVersion.fromInt(1)
        val grpcTx = setAssetScriptTransaction {
    ...
    @Test
    fun `should map to SetAssetScriptTx (with nulls)`() {
        val txVersion = TxVersion.fromInt(1)
        val grpcTx = setAssetScriptTransaction {
            id = byteString("DnK5Xfi2wXUJx9BjK9X6ZpFdTLdq2GtWH9pWrcxcmrhB")
            chainId = 73
